 ...
 */
export function init_fractal_trees(canvas_id) {
    const ptr0 = passStringToWasm0(canvas_id, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.init_fractal_trees(ptr0, len0);
    if (ret[1]) {
        throw takeFromExternrefTable0(ret[0]);
    }
}

export function start_fractal_trees() {
    wasm.start_fractal_trees();
}

export function stop_fractal_trees() {
    wasm.stop_fractal_trees();
}
 ...
 * @param {number} length
 */
export function update_fractal_params(iterations, angle, length) {
    wasm.update_fractal_params(iterations, angle, length);
}
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('fractal_trees_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
